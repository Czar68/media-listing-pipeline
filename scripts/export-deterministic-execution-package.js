...
try {
  ...
  if (err && err.code === 'ENOENT') {
    console.error(`Deterministic execution contract not found (expected at script-relative path): ${contractPath}`);
  } else {
    console.error(`Cannot read deterministic execution contract at ${contractPath}: ${err && err.message ? err.message : err}`);
  }
  process.exit(1);
}
...
for (const key of REQUIRED_TOP_LEVEL) {
  ...
  packagePayload[key] = { relativePath: expected };
}
