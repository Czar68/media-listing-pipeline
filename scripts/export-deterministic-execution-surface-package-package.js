...
try {
  ...
  if (err && err.code === 'ENOENT') {
    console.error(`Surface package contract not found (expected at script-relative path): ${contractPath}`);
  } else {
    console.error(`Cannot read surface package contract at ${contractPath}: ${err && err.message ? err.message : err}`);
  }
  process.exit(1);
}
