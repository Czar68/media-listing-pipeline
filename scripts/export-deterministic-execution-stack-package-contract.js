...
try {
  ...
  if (err && err.code === 'ENOENT') {
    console.error(`Stack package not found (expected at script-relative path): ${packagePath}`);
  } else {
    console.error(`Cannot read stack package at ${packagePath}: ${err && err.message ? err.message : err}`);
  }
  process.exit(1);
}
...
try {
  ...
  console.error(`Invalid JSON in ${PACKAGE_FILE}: ${err && err.message ? err.message : err}`);
  process.exit(1);
}
...
for (const key of REQUIRED_TOP_LEVEL) {
  ...
  if (!isPlainObject(entry)) {
    console.error(`${PACKAGE_FILE} key "${key}" must be a plain object.`);
    process.exit(1);
  }

  const subKeys = Object.keys(entry);
  if (subKeys.length !== 1 || subKeys[0] !== 'relativePath') {
    console.error(`${PACKAGE_FILE} key "${key}" must be an object with exactly one property: relativePath.`);
    process.exit(1);
  }
  ...
  if (expected === null) {
    if (got !== null) {
      console.error(
        `${PACKAGE_FILE} key "${key}" relativePath must be null; got ${JSON.stringify(got)}.`
      );
      process.exit(1);
    }
  } else if (got !== expected) {
    console.error(
      `${PACKAGE_FILE} key "${key}" relativePath must be ${JSON.stringify(expected)}; got ${JSON.stringify(got)}.`
    );
    process.exit(1);
  }

  contractPayload[key] = { relativePath: expected === null ? null : expected };
}

const extraKeys = Object.keys(parsed).filter((k) => !REQUIRED_TOP_LEVEL.includes(k));
if (extraKeys.length > 0) {
  console.error(`${PACKAGE_FILE} has unexpected top-level key(s): ${extraKeys.join(', ')}`);
  process.exit(1);
}
