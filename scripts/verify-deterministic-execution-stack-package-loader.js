...
const ARTIFACT = 'media-listing-deterministic-execution-stack-package.json';
...
function main() {
  ...
  try {
    ...
    if (err && err.code === 'ENOENT') {
      console.error(`Deterministic execution stack package not found (expected at script-relative path): ${artifactPath}`);
    } else {
      console.error(`Cannot read deterministic execution stack package at ${artifactPath}: ${err && err.message ? err.message : err}`);
    }
    process.exit(1);
  }
  ...
  for (const key of REQUIRED_TOP_LEVEL) {
    ...
    if (expected === null) {
      if (got !== null) {
        console.error(
          `${ARTIFACT} key "${key}" relativePath must be null (build-only entry); got ${JSON.stringify(got)}.`
        );
        process.exit(1);
      }
    } else {
