...
try {
  ...
  if (err && err.code === 'ENOENT') {
    console.error(`Surface package not found (expected at script-relative path): ${packagePath}`);
  } else {
    console.error(`Cannot read surface package at ${packagePath}: ${err && err.message ? err.message : err}`);
  }
  process.exit(1);
}
...
for (const key of REQUIRED_TOP_LEVEL) {
  ...
  contractPayload[key] = { relativePath: expected };
}
