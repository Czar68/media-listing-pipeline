...
## PHASE_A_1_EBAY_SANDBOX_CREDENTIAL_VALIDATION (2026-05-11)

**Append-only note:** OBJECTIVE: Configure and verify all environment variables required for eBay sandbox publishing via `scripts/live-publish-verify.js` and EbayExecutor. WORK COMPLETED: Read and reconciled `.env.example`, `auth/ebayConfig.js`, `ebayExecutor.ts`, and `scripts/live-publish-verify.js` to produce definitive env var checklist. Confirmed `EXECUTION_MODE=sandbox` routes to EbayExecutor (not MockExecutor). Confirmed `ENABLE_SANDBOX=true` is required by `environmentGuard.ts`. Confirmed `EBAY_ENV=sandbox` is forced by verify script before runBatch. Confirmed `EBAY_ACCESS_TOKEN` is the canonical access token slot (not `EBAY_ACCESS_TOKEN_SANDBOX`); auth layer refreshes it from sandbox OAuth trio. Confirmed `EBAY_LISTING_CATEGORY_ID=111422` (Video Games) is set and correct. Confirmed no PHASE_A block previously existed in PROJECT_STATE.md. ENV VARS CONFIRMED REQUIRED (non-empty before verify script will pass): From live-publish-verify.js gate — EBAY_ENV, EBAY_ACCESS_TOKEN, EBAY_MERCHANT_LOCATION_KEY, EBAY_FULFILLMENT_POLICY_ID, EBAY_PAYMENT_POLICY_ID, EBAY_RETURN_POLICY_ID. From assertEbaySandboxCredentialsPresent() — EBAY_CLIENT_ID_SANDBOX, EBAY_CLIENT_SECRET_SANDBOX, EBAY_REFRESH_TOKEN_SANDBOX. From environmentGuard.ts / pluggableExecutor.ts — EXECUTION_MODE=sandbox, ENABLE_SANDBOX=true. KNOWN DRIFT: EBAY_ACCESS_TOKEN_SANDBOX exists in local .env but is not read by auth/ebayConfig.js; canonical slot is EBAY_ACCESS_TOKEN only. EBAY_PROD_* keys exist in local .env but are not activated at this phase. NEXT GATE — PHASE_A_2: Build adapters/media-pipeline (dist/ must be current). Run `node scripts/live-publish-verify.js`. Pass: exit 0 + VERDICT: PASS + listingId present in output. URL: `https://www.sandbox.ebay.com/itm/<listingId>` must resolve.
